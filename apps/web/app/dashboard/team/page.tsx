...
import {
  Users, Award, Calendar, Heart, Shield, TrendingUp, Clock, Star,
  MessageSquare, Gift, CheckCircle, AlertCircle, ChevronDown, ChevronUp, Plus,
} from 'lucide-react';
...
const TEAM: TeamMember[] = [
  {
    ...
    kudos: 14, safetyIncidents: 0,
    trainings: [{ name: 'Leadership Workshop', completedAt: new Date('2025-11-15') }],
    performanceRating: 5, milestones: ['30 years at WCC', 'Business of the Year 2017'],
  },
  {
    id: 'EMP-002', name: 'Kyle Richardson', role: 'Operations Supervisor', department: 'Operations',
    startDate: new Date('2015-04-12'), email: 'kyle@wc-con.com', phone: '(780) 865-0068', status: 'active',
    certifications: [
    ...
    kudos: 8, recentKudos: 'Great job coordinating the Jasper road project last week — zero delays!',
    safetyIncidents: 0,
    trainings: [
    ...
    performanceRating: 4.5, milestones: ['10 years at WCC (2025)'],
  },
  {
    id: 'EMP-003', name: 'James Whitebear', role: 'Heavy Equipment Operator', department: 'Operations',
    startDate: new Date('2019-07-08'), email: 'james@wc-con.com', phone: '(780) 555-4455', status: 'active',
    certifications: [
    ...
    kudos: 6, recentKudos: 'Excellent work on the Grande Cache site — ahead of schedule.',
    safetyIncidents: 0,
    trainings: [{ name: 'Dozer Operations', completedAt: new Date('2025-05-12') }],
    upcomingTrainings: ['Crusher Operations Cross-Training (Spring 2026)'],
    performanceRating: 4, milestones: [],
  },
  {
    id: 'EMP-004', name: 'Brandon Flett', role: 'Truck Driver — Class 1', department: 'Transportation',
    startDate: new Date('2021-03-15'), email: 'brandon@wc-con.com', phone: '(780) 555-6677', status: 'active',
    certifications: [
      { name: 'Class 1 Driver\'s Licence', status: 'current' },
    ...
    kudos: 5, safetyIncidents: 0,
    trainings: [{ name: 'Defensive Driving', completedAt: new Date('2025-09-10') }],
    upcomingTrainings: ['COR Safety Renewal (April 2026)'],
    performanceRating: 4, milestones: ['5 years at WCC (2026)'],
  },
  {
    id: 'EMP-005', name: 'Ashley Drummond', role: 'Office Administrator', department: 'Administration',
    startDate: new Date('2022-09-01'), email: 'ashley@wc-con.com', phone: '(780) 865-3000', status: 'training',
    certifications: [{ name: 'WHMIS', status: 'current', expiresAt: new Date('2027-09-01') }],
    kudos: 7, recentKudos: 'Streamlined the quote workflow — saving everyone time!',
    safetyIncidents: 0,
    trainings: [
    ...
    performanceRating: 4.5, milestones: [],
  },
];
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-1">Employee profiles, certifications, training, and recognition</p>
        </div>
        <div className="flex gap-3">
          <button className="btn btn-outline"><Gift className="w-4 h-4 mr-2" />Give Kudos</button>
          <button className="btn btn-dashboard-primary"><Plus className="w-4 h-4 mr-2" />Add Team Member</button>
        </div>
      </div>
        ...
        {[
          ...
          <button key={tab.key} onClick={() => setView(tab.key as typeof view)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              view === tab.key ? 'bg-brand-primary text-white' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50'
            }`}>
            {tab.label}
          </button>
        ))}
      ...
      {view === 'team' && (
        <div className="space-y-3">
          {TEAM.map((member) => {
            ...
            const tenure = Math.round((Date.now() - member.startDate.getTime()) / (365.25 * 24 * 60 * 60000));
            return (
              <div key={member.id} className="card card-hover">
                <div className="flex items-center gap-4 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : member.id)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <StatusBadge status={member.status === 'active' ? 'active' : member.status === 'training' ? 'sent' : 'pending'}>
                        {member.status === 'on_leave' ? 'On Leave' : member.status}
                      </StatusBadge>
                    </div>
                    <p className="text-sm text-gray-600">{member.role} — {member.department}</p>
                    <p className="text-xs text-gray-500 mt-1">{tenure} {tenure === 1 ? 'year' : 'years'} at WCC — Started {format(member.startDate, 'MMM yyyy')}</p>
                  </div>
                  <div className="flex items-center gap-4 text-sm text-gray-500 flex-shrink-0">
                    <span className="flex items-center gap-1" title="Kudos"><Star className="w-4 h-4 text-yellow-500" />{member.kudos}</span>
                    <span className="flex items-center gap-1" title="Certifications"><Shield className="w-4 h-4 text-green-500" />{member.certifications.length}</span>
                    {member.performanceRating && (
                      <span className="flex items-center gap-1" title="Performance"><TrendingUp className="w-4 h-4 text-blue-500" />{member.performanceRating}/5</span>
                    )}
                  </div>
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-200 space-y-4">
                    <div className="grid md:grid-cols-3 gap-4">
                      <div className="bg-gray-50 rounded-lg p-4">
                        <h4 className="font-bold text-sm text-gray-900 mb-3 flex items-center gap-2"><Shield className="w-4 h-4 text-green-600" />Certifications</h4>
                        <ul className="space-y-2">
                          {member.certifications.map((cert) => (
                            <li key={cert.name} className="flex items-center gap-2 text-sm">
                              <span className={`w-2 h-2 rounded-full flex-shrink-0 ${cert.status === 'current' ? 'bg-green-500' : cert.status === 'expiring' ? 'bg-yellow-500' : 'bg-red-500'}`} />
                              <span className="text-gray-700">{cert.name}</span>
                              {cert.expiresAt && <span className="text-xs text-gray-400 ml-auto">{format(cert.expiresAt, 'MMM yyyy')}</span>}
                            </li>
                          ))}
                        </ul>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-4">
                        <h4 className="font-bold text-sm text-gray-900 mb-3 flex items-center gap-2"><Award className="w-4 h-4 text-blue-600" />Training</h4>
                        <ul className="space-y-2">
                          {member.trainings.map((t) => (
                            <li key={t.name} className="text-sm text-gray-700">
                              <span className="font-medium">{t.name}</span>
                              <span className="text-xs text-gray-400 block">Completed {format(t.completedAt, 'MMM yyyy')}</span>
                            </li>
                          ))}
                        </ul>
                        {member.upcomingTrainings && member.upcomingTrainings.length > 0 && (
                          <div className="mt-3 pt-3 border-t border-gray-200">
                            ...
                            {member.upcomingTrainings.map((t) => <p key={t} className="text-xs text-gray-600">{t}</p>)}
                          </div>
                        )}
                      </div>
                      <div className="bg-gray-50 rounded-lg p-4">
                        ...
                        {member.milestones.length > 0 && (
                          <div className="mt-3 pt-3 border-t border-gray-200">
                            ...
                            {member.milestones.map((m) => (
                              <p key={m} className="text-xs text-gray-600 flex items-center gap-1"><Award className="w-3 h-3 text-yellow-500" />{m}</p>
                            ))}
                          </div>
                        )}
                      </div>
        ...
        <div className="space-y-4">
          {expiringCerts.length > 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-5 mb-6">
              <h3 className="font-bold text-yellow-800 mb-3 flex items-center gap-2"><AlertCircle className="w-5 h-5" />Certifications Expiring Within 90 Days</h3>
                ...
                {expiringCerts.map((cert, i) => (
                  <div key={i} className="flex items-center justify-between bg-white rounded-lg p-3 border border-yellow-200">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium text-yellow-700">Expires {cert.expiresAt ? format(cert.expiresAt, 'MMM d, yyyy') : 'N/A'}</p>
                      <p className="text-xs text-gray-500">{cert.expiresAt ? `${differenceInDays(cert.expiresAt, new Date())} days remaining` : ''}</p>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  m.certifications.map((c, i) => (
                    <tr key={`${m.id}-${i}`} className="table-row">
                      ...
                      <td className="px-4 py-3">
                        <StatusBadge status={c.status === 'current' ? 'active' : c.status === 'expiring' ? 'pending' : 'expired'}>
                          {c.status}
                        </StatusBadge>
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-500">{c.expiresAt ? format(c.expiresAt, 'MMM d, yyyy') : 'N/A'}</td>
                    </tr>
                  ))
                )}
              </tbody>
      ...
      {view === 'recognition' && (
        <div className="space-y-6">
          <div className="bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-xl p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">Celebrate your team&apos;s wins — kudos, milestones, and achievements</p>
            <button className="btn btn-dashboard-primary"><Gift className="w-4 h-4 mr-2" />Give Kudos to a Team Member</button>
          </div>
              ...
              {[...TEAM].sort((a, b) => b.kudos - a.kudos).map((member, i) => (
                <div key={member.id} className="flex items-center gap-4 p-3 rounded-lg hover:bg-gray-50">
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                    i === 0 ? 'bg-yellow-100 text-yellow-800' : i === 1 ? 'bg-gray-100 text-gray-800' : i === 2 ? 'bg-orange-100 text-orange-800' : 'bg-gray-50 text-gray-600'
                  }`}>{i + 1}</span>
