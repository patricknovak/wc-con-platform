...
const recentActivity = [
  { id: 1, event: 'Quote QT-2024-156 sent to Apex Construction', timestamp: new Date(Date.now() - 30 * 60000) },
  { id: 2, event: 'AI assessed resume: Dave Makenzie (Crusher Operator) — Score 95', timestamp: new Date(Date.now() - 60 * 60000) },
  { id: 6, event: 'Chat widget converted visitor to quote request (driveway project)', timestamp: new Date(Date.now() - 90 * 60000) },
  { id: 3, event: 'Quote QT-2024-155 accepted by Twin Rivers Ltd', timestamp: new Date(Date.now() - 3 * 60 * 60000) },
  { id: 4, event: 'Hub listing for Cascade Materials approved', timestamp: new Date(Date.now() - 5 * 60 * 60000) },
  { id: 5, event: 'Intelligence system learned pricing for 142 new items', timestamp: new Date(Date.now() - 8 * 60 * 60000) },
];
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              {priorityActions.map((action) => (
                <div key={action.id} className="flex items-start gap-4 p-4 border border-gray-200 rounded-lg hover:shadow-sm transition-shadow">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-medium text-gray-900">{action.title}</h3>
                      <StatusBadge status={action.priority}>{action.priority}</StatusBadge>
                    </div>
