...
const KPIs = [
  { label: 'Monthly Revenue', value: '$245,300', change: '+12%', changeDirection: 'up' as const, icon: '💰' },
  { label: 'Quotes Sent', value: '34', change: '+8%', changeDirection: 'up' as const, icon: '📄' },
  { label: 'Win Rate', value: '32%', change: '+3%', changeDirection: 'up' as const, icon: '🎯' },
  { label: 'Avg Response Time', value: '8 min', change: '-2 min', changeDirection: 'down' as const, icon: '⏱️' },
];
    ...
    <div className="p-8">
          ...
          {periods.map((period) => (
            <button key={period.value} onClick={() => setTimePeriod(period.value)}
              className={`px-4 py-2 rounded text-sm font-medium transition-colors ${
                timePeriod === period.value ? 'bg-brand-primary text-white' : 'text-gray-700 hover:bg-gray-100'
              }`}>
              {period.label}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {KPIs.map((kpi, idx) => (
          <div key={idx} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <p className={`text-sm font-medium ${kpi.changeDirection === 'up' ? 'text-green-600' : 'text-blue-600'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {chartPlaceholders.map((chart, idx) => (
          <div key={idx} className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            <h2 className="text-lg font-bold text-gray-900 mb-6">{chart.title}</h2>
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {[
              ...
              <div key={idx}>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div className="bg-brand-primary rounded-full h-2" style={{ width: `${service.pct}%` }}></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-bold text-gray-900 mb-4">Quote Performance</h3>
          <div className="space-y-4">
            {[
              { label: 'Total Sent', value: '34', color: 'text-gray-900' },
              { label: 'Accepted', value: '11', color: 'text-green-600' },
              { label: 'Pending Review', value: '8', color: 'text-yellow-600' },
              { label: 'Expired/Rejected', value: '15', color: 'text-red-600' },
            ].map((item, idx) => (
